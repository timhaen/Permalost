...
function loadPermafrostData(year) {
  d3.csv("../data/alt_future_1990_2100.csv").then(function (data) {
    ...
    // Calculate average values for each RCP
    const avgRcp26 = d3.mean(filteredData, (d) => d.rcp26);
    const avgRcp45 = d3.mean(filteredData, (d) => d.rcp45);
    const avgRcp85 = d3.mean(filteredData, (d) => d.rcp85);

    // Log the average values to the console
    console.log(`Year: ${year}`);
    console.log(`Average RCP26: ${avgRcp26}`);
    console.log(`Average RCP45: ${avgRcp45}`);
    console.log(`Average RCP85: ${avgRcp85}`);
